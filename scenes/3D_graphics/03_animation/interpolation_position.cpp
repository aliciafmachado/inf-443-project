...
void scene_model::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    if (idx == 0) {
        ...
        const float t0 = keyframes[N-1].t; // = t_{i-1}
        const vec3& p0 = keyframes[N-1].p; // = p_{i-1}
        const float t3 = keyframes[2].t; // = t_{i+2}
        const vec3& p3 = keyframes[2].p; // = p_{i+2}

        const float K = 0.5;
        p = cardinal_spline_interpolation(t, t0, t1, t2, t3, p0, p1, p2, p3, K);
    }
    else if (idx == (keyframes.size() - 2)){
        // Compute the cardinal spline interpolation
        const float t0 = keyframes[idx-1].t; // = t_{i-1}
        const vec3& p0 = keyframes[idx-1].p; // = p_{i-1}
        const float t3 = keyframes[0].t; // = t_{i+2}
        const vec3& p3 = keyframes[0].p; // = p_{i+2}
